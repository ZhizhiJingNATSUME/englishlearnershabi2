...
def generate_qwen_image(prompt: str, seed: int) -> bytes:
    """使用 Qwen 图像模型生成图片数据"""
...
def ensure_article_image(session, article: Article) -> str:
    """确保文章有配图 URL"""
    prompt = build_image_prompt(article)
    prompt_digest = int(hashlib.sha1(prompt.encode("utf-8")).hexdigest()[:8], 16)
    ...
    image_bytes = generate_qwen_image(prompt, prompt_digest)
    if image_bytes:
    ...
    try:
        ...
        if translation:
            try:
                ...
                pass
            article = session.query(Article).filter_by(id=article_id).first()
            translation_paragraphs = split_translation_text(translation.translation_text)
            pairs = build_translation_pairs(article.content if article else "", translation_paragraphs)
            return jsonify({
                'article_id': article_id,
                'target_language': target_lang,
                'paragraphs': pairs
